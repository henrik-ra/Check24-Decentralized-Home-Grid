...
function getSpeedboatUrl(): string {
  return normalizeBaseUrl(import.meta.env.VITE_SPEEDBOAT_URL) || 'http://localhost:3003';
}
  ...
  const simulateInterest = async (clickedOfferId?: string, options?: { keepalive?: boolean; silent?: boolean }) => {
    ...
    try {
      const response = await fetch(`${speedboatUrl}/api/simulate/interest`, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ email: effectiveEmail, vertical: 'insurance', offerId: clickedOfferId }),
        ...
        <Flex direction="column" gap="4">
          <Card size="4">
            <Flex direction="column" gap="3">
              <Heading size="5">{offerId ? `Versicherung ${offerId}` : 'Versicherungen'}</Heading>
              <Text size="2" color="gray">
                Minimaler Product-Site-PoC: sendet ein Interest-Signal an die Insurance-Speedboat und pusht ein Widget in Home.
              </Text>
          ...
          <Card size="3">
            <Flex direction="column" gap="2">
              <Heading size="4">Mock-Versicherungen</Heading>
              <Text size="2" color="gray">
                  ...
                  <Card key={o.id} size="2" style={{ cursor: 'pointer' }} onClick={() => openOffer(o.id)}>
                    <Flex direction="column" gap="2">
                      ...
                      <Flex align="center" justify="between" gap="2">
                        <Badge color="violet">INSURANCE</Badge>
                        <Button
                          size="1"
                          variant="soft"
                          onClick={(e) => {
